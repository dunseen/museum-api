...
const compareTaxons = (
  ...
  from: { id: string; name: string; hierarchy?: string };
  to: { id: string; name: string; hierarchy?: string };
} | null => {
  ...
  return {
    from: {
      ...
      hierarchy: original[0].hierarchy?.name || 'Unknown',
    },
    to: {
      id: updated[0].id,
      name: updated[0].name,
      hierarchy: updated[0].hierarchy?.name || 'Unknown',
    },
  };
};
